...
It check that center of window has minmal/maximal value around window
"""
...
class SMMD:
    ...
    def push(self, pv: PositionalValue):
        tmp = None
    ...
    def pop(self):
        if len(self.stack2) == 0:
            ...
            # it happends 1 times every 2*tail+1 pushes
    ...
    def isCenterMM(self) -> t.Tuple[t.Optional[PositionalValue], bool, bool]:
        """
        return tuple: value/None, True if center is minimal, True if center is maximal
        """
        if len(self.stack1)+len(self.stack2) < self.maxlen:
        ...
        ismin = True
        ismax = True
        isdif1 = False # do we have a difference between min and max is stack1?
        isdif2 = False # do we have a difference between min and max is stack2?
